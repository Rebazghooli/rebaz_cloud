...
ASK_NAME = 10; ASK_FOLDER = 11; ASK_NEW_FOLDER_NAME = 12; ASK_DESC = 13; ASK_TAGS = 14
...
async def upload_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_authed(user.id):
        await query.edit_message_text(t("not_authed", lang)); return ConversationHandler.END
    ...
    await query.edit_message_text(t("send_file", lang))
    return ASK_NAME
...
async def receive_file(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    lang = get_lang(user.id)
    if not is_authed(user.id):
        await update.message.reply_text(t("not_authed", lang)); return ConversationHandler.END
    count = count_user_files(user.id)
    limit = get_file_limit(user.id)
    if count >= limit:
    ...
    await msg.reply_text(t("ask_file_name", lang), reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
    return ASK_NAME
...
async def receive_name(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(t("ask_folder", lang), reply_markup=folder_kb(lang, get_folders(user.id)))
    return ASK_FOLDER
...
async def handle_folder_choice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "folder_new":
        kb = [[InlineKeyboardButton(t("btn_cancel", lang), callback_data="upload_cancel")]]
        await query.edit_message_text(t("ask_new_folder_name", lang), reply_markup=InlineKeyboardMarkup(kb))
        return ASK_NEW_FOLDER_NAME
    elif data == "folder_none":
        upload["folder_id"] = None; upload["folder_name"] = "—"
    ...
    ctx.user_data["upload"] = upload
    kb = [[InlineKeyboardButton(t("btn_cancel", lang), callback_data="upload_cancel")]]
    await query.edit_message_text(t("ask_description", lang), reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
    return ASK_DESC
...
async def receive_new_folder_name(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not create_folder(user.id, name):
        await update.message.reply_text(t("folder_exists", lang)); return ASK_NEW_FOLDER_NAME
...
async def receive_description(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ctx.user_data["upload"] = upload
    kb = [[InlineKeyboardButton(t("btn_cancel", lang), callback_data="upload_cancel")]]
    await update.message.reply_text(t("ask_tags", lang), reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
    return ASK_TAGS
...
async def do_upload(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        sent = await orig_msg.copy_to(chat_id=STORAGE_CHANNEL, caption=caption, parse_mode="HTML")
        channel_msg_id = sent.message_id
    except Exception as e:
        await update.message.reply_text(f"❌ خطا: {e}"); return

    code = save_file(user.id, upload.get("custom_name",""), upload.get("folder_id"),
