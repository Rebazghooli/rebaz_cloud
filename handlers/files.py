...
EDIT_WAIT_NAME=20; EDIT_WAIT_DESC=21; EDIT_WAIT_TAGS=22; EDIT_WAIT_FOLDER=23; SEARCH_WAIT=30
...
async def my_files(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if nf: kb.append([InlineKeyboardButton(f"📭 بدون فولدر ({len(nf)})", callback_data="browse_none")])
    kb.append([InlineKeyboardButton(t("btn_back", lang), callback_data="menu_back")])
    if len(kb) == 1:
        await query.edit_message_text(t("no_folders", lang), reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
        return
    await query.edit_message_text(t("my_files_title", lang), reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
...
async def browse_folder(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not files:
        ...
        return
    await query.edit_message_text(f"📂 <b>{fname}</b> — {len(files)} فایل",
        parse_mode="HTML", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(t("btn_back",lang), callback_data="menu_files")]]))
    for file in files:
        text = t("file_item", lang, name=file["custom_name"], tags=file["tags"] or "—",
                 code=file["unique_code"], date=fmt(file["uploaded_at"]))
        kb = [[InlineKeyboardButton(t("btn_get_file",lang), callback_data=f"get_{file['id']}"),
...
async def get_file_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not file: return
    try:
        await query.message.bot.forward_message(chat_id=user.id, from_chat_id=STORAGE_CHANNEL, message_id=file["channel_msg_id"])
...
async def del_ask(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [[InlineKeyboardButton(t("btn_yes_delete",lang), callback_data=f"delok_{fid}")],
...
async def edit_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [[InlineKeyboardButton(t("btn_edit_name",lang), callback_data="ef_name"),
...
async def edit_ask(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "ef_name":
        ...
        await query.edit_message_text(t("ask_folder",lang), reply_markup=folder_kb(lang, get_folders(user.id)))
...
async def edit_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(t("edit_done",lang), reply_markup=main_menu_kb(lang, is_admin=(user.id==ADMIN_ID)))
...
async def edit_folder_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    data  = query.data
    fid   = ctx.user_data.get("edit_fid")
    if data == "folder_none":     update_file(fid, user.id, folder_id=None)
    elif data.startswith("folder_") and data != "folder_new":
        update_file(fid, user.id, folder_id=int(data.replace("folder_","")))
    from handlers.auth import main_menu_kb
    await query.edit_message_text(t("edit_done",lang), reply_markup=main_menu_kb(lang, is_admin=(user.id==ADMIN_ID)))
    return ConversationHandler.END
...
async def search_recv(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    from handlers.auth import main_menu_kb
    if not results:
        await update.message.reply_text(t("no_results",lang), reply_markup=main_menu_kb(lang, is_admin=(user.id==ADMIN_ID)))
        return ConversationHandler.END
